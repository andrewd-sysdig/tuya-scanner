...
func (dm *DeviceManager) getAppliance(id, version string) *Appliance {
...
func (dm *DeviceManager) Configure(jdata string) {
	...
	for _, c := range conf {
		...
		d := dm.getAppliance(c.GwId, c.Version)
		d.GwId = c.GwId
		d.key = []byte(c.Key)
		if len(c.Ip) > 0 {
			d.Ip = c.Ip
...
func (dm *DeviceManager) DefineDevice(Name, GwId, Key, Ip, Version string, device Device) {
	...
	d := dm.getAppliance(GwId, Version)
	d.GwId = GwId
	d.key = []byte(Key)
	if len(Ip) > 0 {
		d.Ip = Ip
...
func (dm *DeviceManager) applianceUpdate(data []byte) {
	...
	dm.Lock()
	defer dm.Unlock()
	d := dm.getAppliance(rd.GwId, rd.Version)
	d.update(&rd)
}
